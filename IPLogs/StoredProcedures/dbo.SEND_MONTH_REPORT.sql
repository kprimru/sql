...
BEGIN
	...
	SET @HEADER = N'ќтчет за мес€ц пополненным клиентам »ѕ сервера с ' + CONVERT(VARCHAR(20), @BEGIN, 104) + ' по ' + CONVERT(VARCHAR(20), @END, 104)
	...
	EXEC msdb.dbo.sp_send_dbmail @profile_name = 'SQLMail',
				...
				@body = @HEADER,
				@query = @SQL,
				@subject='ћес€чный отчет по »ѕ серверу',
