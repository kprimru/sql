...
GO
IF OBJECT_ID('[dbo].[PROCESS_FILES]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[PROCESS_FILES]  AS SELECT 1')
GO
...
BEGIN
	...
	BEGIN TRY
	    ...
	    BEGIN
		    SET @FILE_FULL = @SERVER_PATH + @FILE_PATH;

		    IF @FILE_TYPE = 'LOG'
		    BEGIN
			    EXEC dbo.LOG_FILE_PROCESS @FILE_PATH, @FILE_SIZE, @SERVER, @FILE_FULL;
		    END
		    ELSE IF @FILE_TYPE = 'CLIENT'
		    BEGIN
			    EXEC dbo.CLIENT_FILE_PROCESS @FILE_PATH, @FILE_SIZE, @SERVER, @FILE_FULL;
		    END
		    ELSE IF @FILE_TYPE = 'SERVER'
		    BEGIN
			    EXEC dbo.SERVER_FILE_PROCESS @FILE_PATH, @FILE_SIZE, @SERVER, @FILE_FULL;
		    END
