...
GO
IF OBJECT_ID('[dbo].[CONSIGNMENT_PRINT_BY_ID_LIST]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[CONSIGNMENT_PRINT_BY_ID_LIST]  AS SELECT 1')
GO
/*
Автор:
Дата создания:  
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO @cons
			SELECT *
			FROM dbo.GET_TABLE_FROM_LIST(@consid, ',')
		...
		CREATE TABLE #master
			(
				...
				CFM_CONSIGN_INN VARCHAR(50),
				CFM_CONSIGN_KPP VARCHAR(50),
				CFM_CONSIGN_OKPO VARCHAR(50),
				CFM_CLIENT_NAME VARCHAR(500),
				CFM_CLIENT_ADDRESS VARCHAR(500),
				CFM_CLIENT_PHONE VARCHAR(50),
				CFM_CLIENT_BANK VARCHAR(500),
		...
		INSERT INTO #master
			(
				...
				CFM_CONSIGN_NAME, CFM_CONSIGN_ADDRESS, CFM_CONSIGN_INN, CFM_CONSIGN_KPP, CFM_CONSIGN_OKPO, CFM_CLIENT_NAME,
				CFM_CLIENT_ADDRESS, CFM_CLIENT_PHONE, CFM_CLIENT_BANK,
				CFM_FOUND, ORG_ID, ORG_SHORT_NAME, ORG_ADDRESS, ORG_INN, ORG_KPP,
				ORG_BANK, ORG_ACCOUNT, ORG_LORO, ORG_BIK, ORG_OKPO, ORG_BUH_SHORT, ORG_DIR_SHORT
			)
		SELECT
			...
			ORG_INN, ORG_KPP,
			BA_NAME AS ORG_BANK, ORG_ACCOUNT, ORG_LORO, ORG_BIK, ORG_OKPO,
		...
		FROM
			...
			dbo.ClientTable ON CL_ID = ISNULL(CSG_ID_PAYER, CSG_ID_CLIENT) LEFT OUTER JOIN 
		...
		SELECT
			(
				SELECT TOP 1 CFM_ID
				FROM #master
				WHERE CSG_ID_CLIENT = CL_ID	AND CSG_NUM = CFM_NUM
			),
			CSG_ID_CLIENT, CSD_NUM, ISNULL(GD_NAME, '') AS CSD_STR,
			ISNULL(SYS_NAME, CSD_NAME) AS CSD_NAME, a.DIS_ID, a.DIS_STR, CSD_CODE,
		...
		ORDER BY CSD_NUM
			...
			INSERT INTO dbo.ConsignmentFactMasterTable
				(
					CFM_NUM, CFM_FACT_DATE, CFM_DATE, CL_ID,
					CFM_CONSIGN_NAME, CFM_CONSIGN_ADDRESS, CFM_CONSIGN_INN, CFM_CONSIGN_KPP, CFM_CONSIGN_OKPO,
					CFM_CLIENT_NAME, CFM_CLIENT_ADDRESS,  CFM_CLIENT_PHONE, CFM_CLIENT_BANK,
					CFM_FOUND, ORG_ID, ORG_SHORT_NAME, ORG_ADDRESS, ORG_INN, ORG_KPP,
					ORG_BANK, ORG_BIK, ORG_LORO, ORG_OKPO,
			...
			INSERT INTO dbo.ConsignmentFactDetailTable
				...
				SELECT
					(
						...
						WHERE CFM_FACT_DATE = @curdate AND CL_ID = CSM_ID_CLIENT AND CSD_NUM = CFM_NUM
