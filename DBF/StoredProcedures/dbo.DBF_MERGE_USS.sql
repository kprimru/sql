...
GO
IF OBJECT_ID('[dbo].[DBF_MERGE_USS]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[DBF_MERGE_USS]  AS SELECT 1')
GO
ALTER PROCEDURE [dbo].[DBF_MERGE_USS]
WITH EXECUTE AS OWNER
AS
BEGIN
	/*
		Слияние базы DBF и [PC276-SQL\USS].[DBF].
	...
	*/
	...
	BEGIN TRY
		...
		FROM [PC276-SQL\USS].[DBF].dbo.ClientTable a
		WHERE NOT EXISTS
			(
				SELECT *
		...
		FROM
			[PC276-SQL\USS].[DBF].dbo.ClientTable a
		...
		WHERE NOT EXISTS
			...
			(
				SELECT *
				FROM [PC276-SQL\USS].[DBF].dbo.TOTable w
			...
			(
				...
				WHERE z.TO_ID_CLIENT = t.CL_ID
					AND NOT EXISTS
						(
							SELECT *
							FROM [PC276-SQL\USS].[DBF].dbo.TOTable y
		...
		INSERT INTO DBF.dbo.CityTable(CT_NAME, CT_PREFIX, CT_REGION)
			...
			FROM [PC276-SQL\USS].[DBF].dbo.CityTable a
			WHERE NOT EXISTS
				(
					SELECT *
		...
		INSERT INTO DBF.dbo.StreetTable(ST_PREFIX, ST_NAME, ST_SUFFIX, ST_ID_CITY, ST_ACTIVE)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.StreetTable c INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.CityTable b ON b.CT_ID = c.ST_ID_CITY
			WHERE NOT EXISTS
				(
					SELECT *
					FROM
		...
		INSERT INTO DBF.dbo.SubhostTable(SH_SHORT_NAME, SH_SUBHOST, SH_LST_NAME, SH_ACTIVE, SH_ORDER)
			...
			FROM [PC276-SQL\USS].[DBF].dbo.SubhostTable a
			WHERE NOT EXISTS
				(
					SELECT *
		...
		FROM
			...
			[PC276-SQL\USS].[DBF].dbo.ClientTable a ON a.CL_NUM = t.CL_NUM LEFT OUTER JOIN
			[PC276-SQL\USS].[DBF].dbo.SubhostTable c ON c.SH_ID = a.CL_ID_SUBHOST LEFT OUTER JOIN
			[PC276-SQL\USS].[DBF].dbo.OrganizationTable d ON d.ORG_ID = a.CL_ID_ORG
		WHERE
				(
		...
		INSERT INTO DBF.dbo.DistrTable(DIS_ID_SYSTEM, DIS_NUM, DIS_COMP_NUM, DIS_ACTIVE)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.DistrTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.SystemTable  b ON b.SYS_ID = DIS_ID_SYSTEM INNER JOIN
		...
		FROM
			...
			[PC276-SQL\USS].[DBF].dbo.DistrTable a ON
								a.DIS_NUM = t.DIS_NUM
							AND a.DIS_COMP_NUM = t.DIS_COMP_NUM INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.SystemTable b ON a.DIS_ID_SYSTEM = b.SYS_ID INNER JOIN
			DBF.dbo.SystemTable c ON c.SYS_ID = t.DIS_ID_SYSTEM INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.HostTable d ON d.HST_ID = b.SYS_ID_HOST INNER JOIN
		...
		INSERT INTO DBF.dbo.DistrServiceStatusTable(DSS_NAME, DSS_ID_STATUS, DSS_REPORT, DSS_ACTIVE)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.DistrServiceStatusTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.DistrStatusTable c ON a.DSS_ID_STATUS = c.DS_ID
			WHERE NOT EXISTS
				(
					SELECT *
		...
		INSERT INTO DBF.dbo.ClientDistrTable(CD_ID_CLIENT, CD_ID_DISTR, CD_REG_DATE, CD_ID_SERVICE)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.ClientDistrTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.ClientTable b ON a.CD_ID_CLIENT = b.CL_ID INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.DistrTable c ON c.DIS_ID = a.CD_ID_DISTR INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.SystemTable d ON d.SYS_ID = c.DIS_ID_SYSTEM INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.DistrServiceStatusTable e ON e.DSS_ID = a.CD_ID_SERVICE
			WHERE NOT EXISTS
				(
					SELECT *
					FROM
		...
		FROM
			[PC276-SQL\USS].[DBF].dbo.ClientDistrTable a INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.ClientTable b ON b.CL_ID = a.CD_ID_CLIENT INNER JOIN
			DBF.dbo.ClientTable c ON c.CL_NUM = b.CL_NUM INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.DistrTable d ON d.DIS_ID = a.CD_ID_DISTR INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.SystemTable e ON e.SYS_ID = d.DIS_ID_SYSTEM INNER JOIN
		...
		INSERT INTO DBF.dbo.AddressTemplateTable(ATL_CAPTION, ATL_INDEX, ATL_COUNTRY, ATL_REGION, ATL_AREA, ATL_CITY_PREFIX, ATL_CITY, ATL_STR_PREFIX, ATL_STREET, ATL_HOME, ATL_ACTIVE)
			...
			FROM [PC276-SQL\USS].[DBF].dbo.AddressTemplateTable a
			WHERE NOT EXISTS
				(
					SELECT *
		...
		INSERT INTO	DBF.dbo.ClientAddressTable(CA_ID_CLIENT, CA_ID_TYPE, CA_INDEX, CA_ID_STREET, CA_HOME, CA_STR, CA_ID_TEMPLATE, CA_FREE)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.ClientAddressTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.ClientTable b ON b.CL_ID = a.CA_ID_CLIENT INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.StreetTable c ON c.ST_ID = a.CA_ID_STREET INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.CityTable d ON d.CT_ID = c.ST_ID_CITY INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.AddressTemplateTable e ON e.ATL_ID = a.CA_ID_TEMPLATE
			WHERE NOT EXISTS
				(
					SELECT *
					FROM
		...
		FROM
			[PC276-SQL\USS].[DBF].dbo.ClientAddressTable a INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.ClientTable b ON a.CA_ID_CLIENT = b.CL_ID INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.StreetTable c ON c.ST_ID = a.CA_ID_STREET INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.CityTable d ON d.CT_ID = c.ST_ID_CITY INNER JOIN
		...
		INSERT INTO DBF.dbo.PositionTable(POS_NAME, POS_ACTIVE)
			...
			FROM [PC276-SQL\USS].[DBF].dbo.PositionTable a
			WHERE NOT EXISTS
				(
					SELECT *
		...
		INSERT INTO DBF.dbo.ClientPersonalTable(PER_ID_CLIENT, PER_FAM, PER_NAME, PER_OTCH, PER_ID_POS, PER_ID_REPORT_POS)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.ClientPersonalTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.ClientTable b ON b.CL_ID = a.PER_ID_CLIENT INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.PositionTable c ON c.POS_ID = a.PER_ID_POS INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.ReportPositionTable d ON d.RP_ID = a.PER_ID_REPORT_POS
			WHERE NOT EXISTS
				(
					SELECT *
					FROM
		...
		FROM
			[PC276-SQL\USS].[DBF].dbo.ClientPersonalTable a INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.ClientTable b ON a.PER_ID_CLIENT = b.CL_ID INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.PositionTable c ON c.POS_ID = a.PER_ID_POS INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.ReportPositionTable d ON d.RP_ID = a.PER_ID_REPORT_POS INNER JOIN
		...
		INSERT INTO DBF.dbo.CourierTable(COUR_NAME, COUR_ACTIVE)
			...
			FROM [PC276-SQL\USS].[DBF].dbo.CourierTable a
			WHERE NOT EXISTS
				(
					SELECT *
		...
		FROM
			...
			[PC276-SQL\USS].[DBF].dbo.TOTable a ON t.TO_NUM = a.TO_NUM INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.ClientTable b ON a.TO_ID_CLIENT = b.CL_ID INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.CourierTable c ON c.COUR_ID  = a.TO_ID_COUR INNER JOIN
			DBF.dbo.ClientTable e ON e.CL_NUM = b.CL_NUM INNER JOIN
		...
		INSERT INTO DBF.dbo.TOAddressTable(TA_ID_TO, TA_INDEX, TA_ID_STREET, TA_HOME)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.TOAddressTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.TOTable b ON a.TA_ID_TO = b.TO_ID INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.StreetTable c ON c.ST_ID = a.TA_ID_STREET INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.CityTable d ON d.CT_ID = ST_ID_CITY
			WHERE NOT EXISTS
				(
					SELECT *
					FROM
		...
		*/
		...
		FROM
			...
			[PC276-SQL\USS].[DBF].dbo.TOTable b ON a.TA_ID_TO = b.TO_ID INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.StreetTable c ON c.ST_ID = a.TA_ID_STREET INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.CityTable d ON c.ST_ID_CITY = d.CT_ID INNER JOIN
			DBF.dbo.CityTable e ON e.CT_NAME = d.CT_NAME INNER JOIN
		...
		INSERT INTO DBF.dbo.TODistrTable(TD_ID_DISTR, TD_ID_TO)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.TODistrTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.TOTable b ON a.TD_ID_TO = b.TO_ID INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.DistrTable c ON c.DIS_ID = a.TD_ID_DISTR INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.SystemTable d ON d.SYS_ID = c.DIS_ID_SYSTEM
			WHERE NOT EXISTS
				(
					SELECT *
					FROM
		...
		FROM
			[PC276-SQL\USS].[DBF].dbo.TODistrTable a INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.TOTable b ON b.TO_ID = a.TD_ID_TO INNER JOIN
			DBF.dbo.TOTable c ON c.TO_NUM = b.TO_NUM INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.DistrTable d ON d.DIS_ID = a.TD_ID_DISTR INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.SystemTable e ON e.SYS_ID = d.DIS_ID_SYSTEM INNER JOIN
			DBF.dbo.SystemTable f ON f.SYS_REG_NAME = e.SYS_REG_NAME INNER JOIN
			DBF.dbo.DistrTable g ON g.DIS_NUM = d.DIS_NUM
								AND g.DIS_COMP_NUM = d.DIS_COMP_NUM
		...
		INSERT INTO DBF.dbo.TOPersonalTable(TP_ID_TO, TP_SURNAME, TP_NAME, TP_OTCH, TP_ID_POS, TP_ID_RP, TP_PHONE, TP_LAST)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.TOPersonalTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.TOTable b ON b.TO_ID = a.TP_ID_TO INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.PositionTable c ON c.POS_ID = a.TP_ID_POS INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.ReportPositionTable d ON d.RP_ID = a.TP_ID_RP
			WHERE NOT EXISTS
				(
					SELECT *
					FROM
		...
		FROM
			[PC276-SQL\USS].[DBF].dbo.TOPersonalTable a INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.TOTable b ON a.TP_ID_TO = b.TO_ID INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.PositionTable c ON c.POS_ID = a.TP_ID_POS INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.ReportPositionTable d ON d.RP_ID = a.TP_ID_RP INNER JOIN
			...
			SELECT
				...
				(
					...
					FROM
						...
						[PC276-SQL\USS].[DBF].dbo.ClientTable y ON z.CL_NUM = y.CL_NUM
				...
				DDH_USER, DDH_DATE
			FROM
				[PC276-SQL\USS].[DBF].dbo.DistrDeliveryHistoryTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.DistrView b ON b.DIS_ID = a.DDH_ID_DISTR
			WHERE NOT EXISTS
				(
					SELECT *
					...
					WHERE a.DDH_USER = z.DDH_USER
						...
						AND z.DDH_ID_OLD_CLIENT =
								(
									...
									FROM
										DBF.dbo.ClientTable t INNER JOIN
										[PC276-SQL\USS].[DBF].dbo.ClientTable q ON t.CL_NUM = q.CL_NUM
		...
		INSERT INTO DBF.dbo.ContractTypeTable(CTT_NAME, CTT_ACTIVE)
			...
			FROM [PC276-SQL\USS].[DBF].dbo.ContractTypeTable a
			WHERE NOT EXISTS
				(
					SELECT *
		...
		INSERT INTO DBF.dbo.ContractPayTable(COP_NAME, COP_DAY, COP_ACTIVE)
			...
			FROM [PC276-SQL\USS].[DBF].dbo.ContractPayTable a
			WHERE NOT EXISTS
				(
					SELECT *
		...
		INSERT INTO DBF.dbo.ContractTable(CO_ID_CLIENT, CO_NUM, CO_ID_TYPE, CO_DATE, CO_BEG_DATE, CO_END_DATE, CO_ACTIVE, CO_ID_PAY)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.ContractTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.ClientTable b ON a.CO_ID_CLIENT = b.CL_ID INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.ContractTypeTable c ON c.CTT_ID = a.CO_ID_TYPE LEFT OUTER JOIN
				[PC276-SQL\USS].[DBF].dbo.ContractPayTable d ON COP_ID = a.CO_ID_PAY
			WHERE NOT EXISTS
				(
					SELECT *
					FROM
		...
		FROM
			...
			[PC276-SQL\USS].[DBF].dbo.ClientTable b ON a.CO_ID_CLIENT = b.CL_ID INNER JOIN
			[PC276-SQL\USS].[DBF].dbo.ContractTypeTable c ON c.CTT_ID = a.CO_ID_TYPE LEFT OUTER JOIN
			[PC276-SQL\USS].[DBF].dbo.ContractPayTable d ON d.COP_ID = a.CO_ID_PAY LEFT OUTER JOIN
		...
		*/
		...
		WHERE NOT EXISTS
			(
				SELECT *
				FROM
					[PC276-SQL\USS].[DBF].dbo.ContractDistrTable a INNER JOIN
					[PC276-SQL\USS].[DBF].dbo.ContractTable b ON b.CO_ID = a.COD_ID_CONTRACT INNER JOIN
					[PC276-SQL\USS].[DBF].dbo.DistrTable c ON c.DIS_ID = a.COD_ID_DISTR INNER JOIN
					[PC276-SQL\USS].[DBF].dbo.SystemTable d ON d.SYS_ID = c.DIS_ID_SYSTEM INNER JOIN
					[PC276-SQL\USS].[DBF].dbo.ClientTable e ON e.CL_ID = b.CO_ID_CLIENT INNER JOIN
		...
		*/
		...
		INSERT INTO DBF.dbo.ContractDistrTable(COD_ID_CONTRACT, COD_ID_DISTR)
			...
			FROM
				[PC276-SQL\USS].[DBF].dbo.ContractDistrTable a INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.ContractTable b ON a.COD_ID_CONTRACT = b.CO_ID INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.DistrTable c ON c.DIS_ID = a.COD_ID_DISTR INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.SystemTable d ON d.SYS_ID = c.DIS_ID_SYSTEM INNER JOIN
				[PC276-SQL\USS].[DBF].dbo.ClientTable e ON e.CL_ID = b.CO_ID_CLIENT INNER JOIN
