...
GO
IF OBJECT_ID('[dbo].[DISTR_FINANCING_REPORT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[DISTR_FINANCING_REPORT]  AS SELECT 1')
GO
				...
				SELECT
					...
					CASE
						...
						WHEN (ISNULL(DF_FIXED_PRICE, 0) <> 0) THEN
							CONVERT(DECIMAL(8, 2), ROUND((100 * (DIS_ORIGIN - DIS_PRICE) / CASE WHEN DIS_ORIGIN = 0 THEN 1 ELSE DIS_ORIGIN END), 2))
