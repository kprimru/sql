...
GO
IF OBJECT_ID('[dbo].[CLIENT_FINANCING_SAVE]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[CLIENT_FINANCING_SAVE]  AS SELECT 1')
GO
ALTER PROCEDURE [dbo].[CLIENT_FINANCING_SAVE]
	...
	@EIS_COMM_CODE VarChar(256) = NULL,
...
BEGIN
	...
	BEGIN TRY
		...
		SET @EIS_COMM_CODE = NULLIF(RTRIM(LTRIM(@EIS_COMM_CODE)), '');
		SET @EIS_CONTRACT = NULLIF(RTRIM(LTRIM(@EIS_CONTRACT)), '');
		SET @EIS_REG_NUM = NULLIF(RTRIM(LTRIM(@EIS_REG_NUM)), '');

		UPDATE dbo.ClientFinancing
		SET BILL_GROUP = @BILL_GROUP,
			BILL_MASS_PRINT = @BILL_MASS_PRINT,
			UNKNOWN_FINANCING = @UNKNOWN,
			EIS_CODE = Ltrim(Rtrim(@EIS_CODE)),
			EIS_COMM_CODE = @EIS_COMM_CODE,
			EIS_DATA = @EIS_DATA,
