...
GO
IF OBJECT_ID('[dbo].[BOOK_PURCHASE_SELECT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[BOOK_PURCHASE_SELECT]  AS SELECT 1')
GO
				...
				SELECT
					...
					(
						...
						WHERE z.ID_PURCHASE = a.ID
							--AND (((S_ALL >= @SUMM_BEGIN) AND (S_ALL >= @SUMM_END))
								--OR ((@SUMM_BEGIN = 0) AND (@SUMM_END = 0)))
					) AS S_ALL,
					CASE
