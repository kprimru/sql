...
GO
IF OBJECT_ID('[dbo].[DBF_MERGE_CHECK_USS]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[DBF_MERGE_CHECK_USS]  AS SELECT 1')
GO
ALTER PROCEDURE [dbo].[DBF_MERGE_CHECK_USS]
	...
	BEGIN TRY
		...
		INSERT INTO @ER_TBL(ERR, ERR_NOTE)
			...
			FROM
				...
				[PC276-SQL\USS].[DBF].dbo.SubhostTable c ON c.SH_ID = a.CL_ID_SUBHOST
			WHERE
					(
			...
			FROM
				...
				INNER JOIN DBF.dbo.DistrView WITH(NOEXPAND) ON CD_ID_DISTR = DIS_ID
			...
			FROM
				...
				[PC276-SQL\USS].[DBF].dbo.AddressTemplateTable e ON e.ATL_ID = a.CA_ID_TEMPLATE INNER JOIN
			...
			*/
			...
			FROM
				...
				INNER JOIN DBF.dbo.DistrView WITH(NOEXPAND) ON DIS_ID = TD_ID_DISTR
