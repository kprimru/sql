...
GO
/*
јвтор:		ƒенисов јлексей/Ѕогдан ¬ладимир
ƒата:		23-04-2009
ќписание:	счет-фактура на аванс за период
*/
...
BEGIN
	...
	BEGIN TRY
		...
		-- провер€ем, есть ли уже (неавансова€) счет-фактура клиенту за этот период
		...
		BEGIN
			...
			SET @idstring = ''
			SELECT	@docstring = @docstring + ('є'+IN_PAY_NUM +' от '+CONVERT(varchar,IN_PAY_DATE,104)) + '; ',
			...
			-- сначала добавл€ем саму счет-фактуру
			...
			-- потом добавл€ем строки таблицы счета-фактуры
			...
			SELECT
					...
					'ѕредоплата за '+SO_INV_STR+' '+C.SYS_NAME,
			...
			-- теперь обрабатываем id-шники платежей
				...
				-- заносим в Income сведени€ о созданной с/ф (в ID_ID_INVOCIE)
