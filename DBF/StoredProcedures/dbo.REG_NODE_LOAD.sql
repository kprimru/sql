...
GO
IF OBJECT_ID('[dbo].[REG_NODE_LOAD]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[REG_NODE_LOAD]  AS SELECT 1')
GO
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO #tmp
			...
			FROM (
				SELECT DISTINCT dbo.GET_HOST_BY_COMMENT2(REG_COMMENT, REG_DISTR, REG_SYSTEM, REG_SYSTEM_TYPE) AS REG_COMMENT, REG_SYSTEM, REG_DISTR
				FROM #temp
				WHERE NOT EXISTS	(
									SELECT *
									FROM dbo.SubhostTable
									WHERE SH_LST_NAME = dbo.GET_HOST_BY_COMMENT2(REG_COMMENT, REG_DISTR, REG_SYSTEM, REG_SYSTEM_TYPE)
										AND SH_REG = 1
									)
				) AS dt
		...
		INSERT INTO #tmp
			...
			FROM
				(
					...
					FROM
						#temp
						INNER JOIN dbo.SubhostTable ON SH_REG = 1 AND SH_LST_NAME = dbo.GET_HOST_BY_COMMENT2(REG_COMMENT, REG_DISTR, REG_SYSTEM, REG_SYSTEM_TYPE)
		...
		ELSE
			...
			*/
			...
			FROM
				...
				INNER JOIN
				dbo.SubhostTable ON SH_REG = 1 AND SH_LST_NAME = dbo.GET_HOST_BY_COMMENT2(REG_COMMENT, REG_DISTR, REG_SYSTEM, REG_SYSTEM_TYPE)
				    /*
					CASE
