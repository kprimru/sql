...
BEGIN	
	...
	IF DB_ID('DBF_NAH') IS NOT NULL
		SELECT 
			TO_REPORT, TO_NUM, TO_NAME, TO_ID, COUR_NAME, TO_MAIN, CL_INN, TO_LAST, (SELECT COUNT(*) FROM DBF_NAH.dbo.TOTable z WHERE z.TO_NUM = a.TO_NUM) AS TO_NAH, TO_PARENT,
	...
	ELSE
		SELECT 
			TO_REPORT, TO_NUM, TO_NAME, TO_ID, COUR_NAME, TO_MAIN, CL_INN, TO_LAST, 0 AS TO_NAH, TO_PARENT,
			ST_CITY_NAME + ', ' + TA_HOME AS TO_ADDRESS
		FROM 
			dbo.TOView a
			LEFT OUTER JOIN dbo.TOAddressView b ON a.TO_ID = b.TA_ID_TO
