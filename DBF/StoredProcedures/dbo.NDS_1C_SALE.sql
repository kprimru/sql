...
GO
IF OBJECT_ID('[dbo].[NDS_1C_SALE]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[NDS_1C_SALE]  AS SELECT 1')
GO
ALTER PROCEDURE [dbo].[NDS_1C_SALE]
	@ORG	SMALLINT,
	@TAX	SMALLINT,
	@PERIOD	SMALLINT
AS
BEGIN
	...
	BEGIN TRY
		...
		FROM
			(
				SELECT
					NUM, DATE, ISNULL(CL_1C, '!!!НЕТ КЛИЕНТА!!!') AS CL_1C,
					(
...
END
GO
GRANT EXECUTE ON [dbo].[NDS_1C_SALE] TO rl_book_sale_p;
GO
