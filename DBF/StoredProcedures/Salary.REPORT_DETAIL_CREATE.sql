...
GO
IF OBJECT_ID('[Salary].[REPORT_DETAIL_CREATE]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Salary].[REPORT_DETAIL_CREATE]  AS SELECT 1')
GO
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			...
			CT_NAME + ' (' + CASE
				WHEN CT_NAME = (SELECT CT_NAME FROM dbo.CityTable INNER JOIN dbo.CourierTable ON COUR_ID_CITY = CT_ID WHERE COUR_ID = a.COUR_ID) THEN 'БГ'
				WHEN CT_NAME <> (SELECT CT_NAME FROM dbo.CityTable INNER JOIN dbo.CourierTable ON COUR_ID_CITY = CT_ID WHERE COUR_ID = a.COUR_ID) THEN 'УТ'
				ELSE '-'
			END + ')' AS CT_NAME,
