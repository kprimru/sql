...
GO
IF OBJECT_ID('[dbo].[INVOICE_CREATE_BY_INCOME]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[INVOICE_CREATE_BY_INCOME]  AS SELECT 1')
GO
/*
Автор:
Дата создания:
Описание:
*/
ALTER PROCEDURE [dbo].[INVOICE_CREATE_BY_INCOME]
	@inid INT,
...
BEGIN
	...
	*/
			...
			FROM
				dbo.ClientTable z
				INNER JOIN dbo.IncomeTable y ON IN_ID_CLIENT = CL_ID
			WHERE IN_ID = @inid
				...
				AND EXISTS
					(
						...
						FROM
							(
								SELECT
									e.ID_ID_DISTR, GD_NAME, SYS_PREFIX, SYS_NAME, e.ID_PRICE, TX_PERCENT, UN_NAME, c.ID_ID_PERIOD, TX_ID, DELTA, SL_REST
								FROM
									dbo.IncomeTable
				...
				FROM
					(
						SELECT ID_ID_DISTR, GD_NAME, SYS_PREFIX, SYS_NAME, ID_PRICE, TX_PERCENT, UN_NAME, ID_ID_PERIOD,	TX_ID,
							CASE WHEN SL_REST >= 0 THEN 0 WHEN ABS(SL_REST) > ID_PRICE THEN ID_PRICE ELSE ABS(SL_REST) END AS DELTA, SL_REST
						FROM
							(
								SELECT
									e.ID_ID_DISTR, GD_NAME, SYS_PREFIX, SYS_NAME, e.ID_PRICE, TX_PERCENT, UN_NAME, c.ID_ID_PERIOD, TX_ID, DELTA, SL_REST
		...
		*/
		INSERT INTO dbo.FinancingProtocol(ID_CLIENT, ID_DOCUMENT, TP, OPER, TXT)
			SELECT INS_ID_CLIENT, INS_ID, 'INVOICE', 'Создание с/ф по оплате', '№' + CONVERT(VARCHAR(20), INS_NUM) + '/' + CONVERT(VARCHAR(20), INS_NUM_YEAR) + ' от ' + CONVERT(VARCHAR(20), INS_DATE, 104)
			FROM
...
END
GO
GRANT EXECUTE ON [dbo].[INVOICE_CREATE_BY_INCOME] TO rl_invoice_w;
GO
