...
GO
IF OBJECT_ID('[dbo].[REPORT_NEW_SYSTEM_NEW]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[REPORT_NEW_SYSTEM_NEW]  AS SELECT 1')
GO
ALTER PROCEDURE [dbo].[REPORT_NEW_SYSTEM_NEW]
			...
			SET @sql = @sql + '
				(
					...
					FROM dbo.PeriodRegNewTable b INNER JOIN
						...
						#dbf_period y ON b.RNN_ID_PERIOD = y.PR_ID INNER JOIN
						dbo.WeightRules w ON w.ID_PERIOD = RNN_ID_PERIOD
										AND w.ID_SYSTEM = RNN_ID_SYSTEM
										AND w.ID_NET = RNN_ID_NET
										AND w.ID_TYPE = RNN_ID_TYPE
		...
		SET @sql = @sql + ' FROM 
			(
				...
				FROM 
					...
					dbo.SystemNetCountTable f ON f.SNC_ID = b.RNN_ID_NET INNER JOIN
					#dbf_systemnet g ON g.TSN_ID = f.SNC_ID_SN INNER JOIN
					#dbf_period y ON z.PR_ID = y.PR_ID INNER JOIN
					dbo.WeightRules w ON w.ID_PERIOD = RNN_ID_PERIOD
										AND w.ID_SYSTEM = RNN_ID_SYSTEM
										AND w.ID_NET = RNN_ID_NET
										AND w.ID_TYPE = RNN_ID_TYPE
