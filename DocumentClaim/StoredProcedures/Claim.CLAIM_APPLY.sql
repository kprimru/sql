...
GO
IF OBJECT_ID('[Claim].[CLAIM_APPLY]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Claim].[CLAIM_APPLY]  AS SELECT 1')
GO
ALTER PROCEDURE [Claim].[CLAIM_APPLY]
	@ID		UNIQUEIDENTIFIER,
	@NOTE	NVARCHAR(MAX)
AS
BEGIN
	...
	BEGIN TRY
		...
		IF (SELECT TOP 1 STATUS FROM Claim.DocumentStatus WHERE ID_DOCUMENT = @ID ORDER BY DATE DESC) <> 1
		BEGIN
			RAISERROR('Ошибка! Статус заказа не подразумевает принятие!', 16, 1)
		END
		ELSE
		BEGIN
			INSERT INTO Claim.DocumentStatus(ID_DOCUMENT, STATUS, ID_AUTHOR, NOTE)
				SELECT @ID,	2, a.ID, @NOTE
				...
				FROM
					...
					CROSS JOIN
						(
							...
							WHERE a.RL_NAME = 'rl_claim_notify_apply'
...
END
GO
GRANT EXECUTE ON [Claim].[CLAIM_APPLY] TO rl_claim_apply;
GO
