	...
	FROM
		(
			SELECT
				...
				Common.ByteToStr((a1.reserved + ISNULL(a4.reserved,0))* 8 * 1024) AS reserved_str,
				a1.data * 8 * 1024 AS data,
				Common.ByteToStr(a1.data * 8 * 1024) AS data_str,
				(
					CASE
				...
				(
					CASE
						WHEN (a1.used + ISNULL(a4.used,0)) > a1.data THEN  Common.ByteToStr(((a1.used + ISNULL(a4.used,0)) - a1.data) * 8 * 1024)
						ELSE '0 б'
					END
				)  AS index_str,
				(
				...
				(
					CASE
						WHEN (a1.reserved + ISNULL(a4.reserved,0)) > a1.used THEN Common.ByteToStr(((a1.reserved + ISNULL(a4.reserved,0)) - a1.used) * 8 * 1024)
						ELSE '0 б'
					END
				) AS unused_str
			FROM
