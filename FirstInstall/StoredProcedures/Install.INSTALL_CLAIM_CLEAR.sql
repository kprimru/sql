...
GO
IF OBJECT_ID('[Install].[INSTALL_CLAIM_CLEAR]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Install].[INSTALL_CLAIM_CLEAR]  AS SELECT 1')
GO

ALTER PROCEDURE [Install].[INSTALL_CLAIM_CLEAR]
	@IND_ID		VARCHAR(MAX)
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN
		...
		EXEC Common.PROTOCOL_INSERT 'INSTALL_DETAIL', 'Удаление заявки', @ID, @OLD, @NEW
