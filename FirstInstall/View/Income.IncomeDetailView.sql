...
GO
IF OBJECT_ID('[Income].[IncomeDetailView]', 'V ') IS NULL EXEC('CREATE VIEW [Income].[IncomeDetailView]  AS SELECT 1')
GO
...
AS
	SELECT
		...
		ID_FULL_DATE, ID_MON_STR, ID_SALARY, CAST (ROUND((ID_SALARY / 1.2), 2) AS MONEY) AS ID_SALARY_NDS,
		ID_ID_FULL_PAY, ID_ID_FIRST_MON, ID_MON_CNT, ID_NOTE,
		ID_SUP_PRICE, CAST(ROUND((ID_SUP_PRICE / 1.2), 2) AS MONEY) AS ID_SUP_PRICE_NDS, ID_SUP_DISCOUNT,
		ID_SUP_MONTH, CAST(ROUND((ID_SUP_MONTH / 1.2), 2) AS MONEY) AS ID_SUP_MONTH_NDS, ID_PREPAY,
		...
		CASE
			...
			ELSE TT_SHORT
		END AS NT_NEW_NAME, ID_MAIN, ID_COLOR, ID_ORANGE,
		ID_INSTALL
	FROM
