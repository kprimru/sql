...
GO
IF OBJECT_ID('[Distr].[DistrLast]', 'V ') IS NULL EXEC('CREATE VIEW [Distr].[DistrLast]  AS SELECT 1')
GO

ALTER VIEW [Distr].[DistrLast]
--WITH SCHEMABINDING
AS
	SELECT
	...
	WHERE DH_REF IN (1, 3) AND HST_REF IN (1, 3) AND SYS_REF IN (1, 3)
		AND NT_REF IN (1, 3) AND DT_REF IN (1, 3) AND TT_REF IN (1, 3)GO
