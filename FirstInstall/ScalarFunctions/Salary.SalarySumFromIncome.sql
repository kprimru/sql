...
GO
IF OBJECT_ID('[Salary].[SalarySumFromIncome]', 'FN') IS NULL EXEC('CREATE FUNCTION [Salary].[SalarySumFromIncome] () RETURNS Int AS BEGIN RETURN NULL END')
GO

CREATE FUNCTION [Salary].[SalarySumFromIncome]
(
...
BEGIN
	...
	FROM
		(
			SELECT
				...
				ID_SUP_MONTH_NDS, ID_SUP_DISCOUNT, ID_PREPAY,
				ID_SUP_CONTRACT, ID_SUP_DATE, SYS_MAIN,
				DT_ID_MASTER, ID_EXCHANGE
			FROM
	...
	BEGIN
		...
		*/
		...
		BEGIN
			IF @BC_SUP_PRICE = 1
				SET @RESULT = ISNULL(@ID_SUP_PRICE, @ID_DEL_SUM)
			ELSE IF @BC_RES_PRICE = 1
				SET @RESULT = @ID_DEL_SUM
