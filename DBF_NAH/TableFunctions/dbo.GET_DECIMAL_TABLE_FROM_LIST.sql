...
GO
IF OBJECT_ID('[dbo].[GET_DECIMAL_TABLE_FROM_LIST]', 'TF') IS NULL EXEC('CREATE FUNCTION [dbo].[GET_DECIMAL_TABLE_FROM_LIST] () RETURNS @output TABLE(Id Int) AS BEGIN RETURN END')
GO
CREATE FUNCTION [dbo].[GET_DECIMAL_TABLE_FROM_LIST](@List VARCHAR(MAX), @Delimiter VARCHAR(10)=',')
RETURNS @tbl TABLE (Item DECIMAL(10, 4) NOT NULL) AS
BEGIN
  IF ISNULL(@List, '') = ''
    RETURN
  DECLARE
