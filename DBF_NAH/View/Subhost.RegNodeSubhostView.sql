...
GO
IF OBJECT_ID('[Subhost].[RegNodeSubhostView]', 'V ') IS NULL EXEC('CREATE VIEW [Subhost].[RegNodeSubhostView]  AS SELECT 1')
GO
...
AS
	SELECT
		...
		SST_ID, SST_CAPTION,
		ISNULL(k.SN_ID, g.SN_ID) AS SN_ID, ISNULL(k.SN_NAME, g.SN_NAME) AS SN_NAME,
	...
	FROM
		Subhost.RegNodeSubhostTable a INNER JOIN
