...
BEGIN
	...
	IF EXISTS
		...
		SELECT
			...
			ROUND(SCR_IC, 2) AS SCR_IC_TOTAL, ROUND(SCR_IC * 0.18, 2) AS SCR_IC_TAX, ROUND(SCR_IC, 2) + ROUND(SCR_IC * 0.18, 2) AS SCR_IC_TOTAL_TAX
		FROM Subhost.SubhostCalcReport
		WHERE SCR_ID_SUBHOST = @SH_ID
			AND SCR_ID_PERIOD = @PR_ID
	ELSE
	...
	SELECT
		...
		SCR_IC AS SCR_IC_TOTAL, ROUND(SCR_IC * 0.18, 2) AS SCR_IC_TAX, SCR_IC + ROUND(SCR_IC * 0.18, 2) AS SCR_IC_TOTAL_TAX
	FROM
		(
			SELECT
				...
				CONVERT(MONEY, ROUND((SCR_TRAFFIC + SCR_DELIVERY - (SCR_IC  /*+ (SELECT SHP_DEBT / 1.18 FROM #tmp_study)*/) + SCR_MARKET + SCR_STUDY + SCR_PAPPER +
					SCR_DELIVERY_SYS + SCR_SUPPORT - SCR_DIU) * 0.18, 2)) AS SCR_NDS_18, 
				SCR_INCOME, SCR_DEBT, SCR_SALDO, SCR_PENALTY
			FROM
				(
					SELECT
