...
GO
IF OBJECT_ID('[dbo].[CONTRACT_KIND_EDIT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[CONTRACT_KIND_EDIT]  AS SELECT 1')
GO
...
BEGIN
	...
	BEGIN TRY
		...
		SET CK_NAME	=	@NAME,
			...
			CK_FOOTER	=	@FOOTER,
			CK_ACTIVE	=	@ACTIVE,
			CK_CREATIVE =   @CREATIVE,
			CK_PREPOSITIONAL = @PREPOSITIONAL
		WHERE CK_ID = @ID
