...
BEGIN
  ...
  --Шаг 1. Создать таблицу со всеми полями (надо чтобы были отсортированы по порядку)
	...
	INSERT INTO #stats
		...
		FROM
			(
				...
				GROUP BY RNN_ID_PERIOD, RNN_ID_SYSTEM, RNN_ID_HOST, SN_GROUP, REG_COMPLECT, TSYS_PROBLEM
			) AS o_O
	...
	INSERT INTO #keys
		SELECT DISTINCT SH_SHORT_NAME + ' | ' + SYS_SHORT_NAME + CASE TSYS_PROBLEM WHEN 0 THEN '' ELSE ' Проблемный' END + ' | ' + SN_GROUP, SH_ID, SYS_ID, SN_GROUP, TSYS_PROBLEM, MIN(SN_ORDER) AS SN_ORD, SYS_ORDER, SH_ORDER
		FROM
			dbo.SystemNetTable a
