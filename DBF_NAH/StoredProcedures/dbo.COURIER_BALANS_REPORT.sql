...
GO
IF OBJECT_ID('[dbo].[COURIER_BALANS_REPORT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[COURIER_BALANS_REPORT]  AS SELECT 1')
GO
					  ...
					  ,CASE
						  ...
						  CASE
							WHEN (P.[REG_ID_SYSTEM] <> O.[REG_ID_SYSTEM]) THEN
							  CASE
								  WHEN T.[SN_COEF] = T_O.[SN_COEF] THEN 'ПОВЫШЕНИЕ '+' с ' + Q_O.[SYS_SHORT_NAME] +' на '+Q.[SYS_SHORT_NAME]
								  ELSE 'ПОВЫШЕНИЕ '+' с ' + Q_O.[SYS_SHORT_NAME] +' на '+Q.[SYS_SHORT_NAME] +' с ' + T_O.[SN_NAME] +' на '+T.[SN_NAME]
                    		  END
							 ELSE 'ПОВЫШЕНИЕ ' +' с ' + T_O.[SN_NAME] +' на '+T.[SN_NAME]
						  END
						  WHEN (((T.[SN_COEF] * S.[STW_WEIGHT])-(T_O.[SN_COEF] * S_O.[STW_WEIGHT]))<0) THEN
						  CASE
							WHEN (P.[REG_ID_SYSTEM] <> O.[REG_ID_SYSTEM]) THEN
							  CASE
								  WHEN T.[SN_COEF] = T_O.[SN_COEF] THEN 'ПОНИЖЕНИЕ '+' с ' + Q_O.[SYS_SHORT_NAME] +' на '+Q.[SYS_SHORT_NAME]
								  ELSE 'ПОНИЖЕНИЕ '+' с ' + Q_O.[SYS_SHORT_NAME] +' на '+Q.[SYS_SHORT_NAME] +' с ' + T_O.[SN_NAME] +' на '+T.[SN_NAME]
                    		  END
							 ELSE 'ПОНИЖЕНИЕ' +' с ' + T_O.[SN_NAME] +' на '+T.[SN_NAME]
						  END
					   END
					,P.REG_COMMENT
				  FROM [dbo].[PeriodRegTable] P
