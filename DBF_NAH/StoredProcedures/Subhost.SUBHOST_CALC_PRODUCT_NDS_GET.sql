...
GO
IF OBJECT_ID('[Subhost].[SUBHOST_CALC_PRODUCT_NDS_GET]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Subhost].[SUBHOST_CALC_PRODUCT_NDS_GET]  AS SELECT 1')
GO
ALTER PROCEDURE [Subhost].[SUBHOST_CALC_PRODUCT_NDS_GET]
	@PR_ID	SMALLINT,
	@SH_ID	SMALLINT,
	@HIDE	BIT = 0
AS
BEGIN
	...
	BEGIN TRY
		...
		WHERE SPG_ID = 1 AND (@HIDE = 0 OR SPC_COUNT <> 0)
...
GO
GRANT EXECUTE ON [Subhost].[SUBHOST_CALC_PRODUCT_NDS_GET] TO rl_subhost_calc;
GO
