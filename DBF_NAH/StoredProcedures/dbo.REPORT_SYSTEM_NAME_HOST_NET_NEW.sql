...
GO
ALTER PROCEDURE [dbo].[REPORT_SYSTEM_NAME_HOST_NET_NEW]
...
BEGIN
    ...
    BEGIN
		...
		INSERT INTO #dbf_system
			SELECT *, CASE
					...
					ELSE 0
				END FROM dbo.GET_TABLE_FROM_LIST(@systemlist, ',')
    END
  ...
  --Шаг 1. Создать таблицу со всеми полями (надо чтобы были отсортированы по порядку)
	...
	INSERT INTO #stats
		(
			PR_ID, SYS_ID, SH_ID, SN_GRP, PROBLEM, CNT
		)
		...
		FROM
			(
						...
						CASE
							WHEN TSYS_PROBLEM = 1
								...
								(
									...
									WHERE a.REG_COMPLECT = b.REG_COMPLECT AND a.REG_ID_PERIOD = b.REG_ID_PERIOD AND DS_REG = 0 AND REG_ID_TYPE <> 6 AND a.REG_ID_SYSTEM <> b.REG_ID_SYSTEM
								) AND EXISTS
								(
									SELECT *
				...
				GROUP BY REG_ID_PERIOD, REG_ID_SYSTEM, REG_ID_HOST, SN_GROUP, REG_COMPLECT, TSYS_PROBLEM
			) AS o_O
	...
	BEGIN
		...
		INSERT INTO #ric
			SELECT DISTINCT 'РИЦ | ' + SYS_SHORT_NAME + CASE TSYS_PROBLEM WHEN 0 THEN '' ELSE ' Проблемный' END + ' | ' + SN_GROUP, 0, SYS_ID, SN_GROUP, TSYS_PROBLEM, MIN(SN_ORDER) AS SN_ORD, SYS_ORDER
			FROM
				...
				INNER JOIN
					(
						SELECT TSYS_ID, 0 AS TSYS_PROBLEM
						FROM #dbf_system

						UNION ALL

						SELECT TSYS_ID, 1 AS TSYS_PROBLEM
						FROM #dbf_system
						WHERE TSYS_PROBLEM = 1
					) d ON d.TSYS_ID = b.SYS_ID
			GROUP BY SYS_SHORT_NAME, TSYS_PROBLEM, SN_GROUP, SYS_ID, SN_GROUP, TSYS_PROBLEM, SYS_ORDER
			ORDER BY SYS_ORDER, TSYS_PROBLEM, SN_ORD
    END


	INSERT INTO #keys
		SELECT SH_SHORT_NAME + ' | ' + SYS_SHORT_NAME + CASE TSYS_PROBLEM WHEN 0 THEN '' ELSE ' Проблемный' END + ' | ' + SN_GROUP, SH_ID, SYS_ID, SN_GROUP, TSYS_PROBLEM, MIN(SN_ORDER) AS SN_ORD, SYS_ORDER, SH_ORDER
		FROM
			dbo.SystemNetTable a
		...
		GROUP BY SH_SHORT_NAME, SYS_SHORT_NAME, TSYS_PROBLEM, SN_GROUP, SH_ID, SYS_ID, SN_GROUP, TSYS_PROBLEM, SYS_ORDER, SH_ORDER
