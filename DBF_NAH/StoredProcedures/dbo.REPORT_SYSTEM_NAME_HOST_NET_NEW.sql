...
GO
IF OBJECT_ID('[dbo].[REPORT_SYSTEM_NAME_HOST_NET_NEW]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[REPORT_SYSTEM_NAME_HOST_NET_NEW]  AS SELECT 1')
GO
		...
		BEGIN
			...
			INSERT INTO #dbf_system
				SELECT Item, CASE
						...
						WHEN SYS_REG_NAME IN ('BBKZ', 'UMKZ', 'UBKZ', 'SPK-I', 'SPK-II', 'SPK-III', 'SPK-IV', 'SPK-V') THEN 2
						ELSE 0
					END FROM dbo.GET_TABLE_FROM_LIST(@systemlist, ',') INNER JOIN dbo.SystemTable ON SYS_ID = Item
		END
