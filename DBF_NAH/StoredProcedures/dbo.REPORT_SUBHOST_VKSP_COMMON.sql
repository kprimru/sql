...
BEGIN
	...
	INSERT INTO #res (SYS, SYS_ORD, SN, SN_ORD, WEIGHT_ONE)
		SELECT DISTINCT SYS_SHORT_NAME, SYS_ORDER, SN_NAME, SN_ORDER, WEIGHT_ONE
		FROM #reg

	SET @SQL = 'UPDATE #res SET '

	SELECT @SQL = @SQL + '[' + SH_SHORT + '|Количество] = (SELECT CNT FROM #reg WHERE SYS_SHORT_NAME = SYS AND SN = SN_NAME AND REG_ID_HOST = ' + CONVERT(VARCHAR(20), SH_ID) + '),'
	FROM @SUBHOST
	ORDER BY SH_ORDER
	...
	SET @SQL = 'SELECT SYS, SN, WEIGHT_ONE, '
	...
	SET @SQL = @SQL + 'FROM #res ORDER BY SYS_ORD, SN_ORD'
