...
GO
IF OBJECT_ID('[Subhost].[SUBHOST_SUPPORT_PRICE_SELECT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Subhost].[SUBHOST_SUPPORT_PRICE_SELECT]  AS SELECT 1')
GO
ALTER PROCEDURE [Subhost].[SUBHOST_SUPPORT_PRICE_SELECT]
	@SH_ID SMALLINT,
	@PR_ID SMALLINT
AS
BEGIN
	...
	BEGIN TRY
		...
		WHERE PT_ID_GROUP IN (5, 7) AND PS_ID_PERIOD = @PR_ID
			AND NOT EXISTS
				(
					SELECT *
...
GO
GRANT EXECUTE ON [Subhost].[SUBHOST_SUPPORT_PRICE_SELECT] TO rl_subhost_calc;
GO
