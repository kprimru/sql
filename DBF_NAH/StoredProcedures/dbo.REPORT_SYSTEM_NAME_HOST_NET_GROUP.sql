...
BEGIN
  ...
  --Шаг 1. Создать таблицу со всеми полями (надо чтобы были отсортированы по порядку)
	...
	INSERT INTO #stats
		...
		FROM
			(
				...
				GROUP BY REG_ID_PERIOD, TSYS_GROUP, REG_ID_HOST, CASE SN_GROUP WHEN 'ОВМ' THEN '1/с' ELSE SN_GROUP END, REG_COMPLECT, TSYS_PROBLEM
			) AS o_O
	...
	BEGIN
		...
		INSERT INTO #ric
			SELECT DISTINCT 'РИЦ | ' + TSYS_GROUP + CASE TSYS_PROBLEM WHEN 0 THEN '' ELSE ' Проблемный' END + ' | ' + CASE SN_GROUP WHEN 'ОВМ' THEN '1/с' ELSE SN_GROUP END, 0, TSYS_GROUP, CASE SN_GROUP WHEN 'ОВМ' THEN '1/с' ELSE SN_GROUP END, TSYS_PROBLEM, MIN(SN_ORDER) AS SN_ORD, MIN(SYS_ORDER) AS SYS_ORDER
			FROM
			...
			GROUP BY TSYS_PROBLEM, CASE SN_GROUP WHEN 'ОВМ' THEN '1/с' ELSE SN_GROUP END, TSYS_GROUP, TSYS_PROBLEM--, SYS_ORDER
			ORDER BY SYS_ORDER, TSYS_PROBLEM, SN_ORD
    END


	INSERT INTO #keys
		SELECT DISTINCT SH_SHORT_NAME + ' | ' + TSYS_GROUP + ' ' + CASE TSYS_PROBLEM WHEN 0 THEN '' ELSE ' Проблемный' END + ' | ' + CASE SN_GROUP WHEN 'ОВМ' THEN '1/с' ELSE SN_GROUP END, SH_ID, TSYS_GROUP, CASE SN_GROUP WHEN 'ОВМ' THEN '1/с' ELSE SN_GROUP END, TSYS_PROBLEM, MIN(SN_ORDER) AS SN_ORD, MIN(SYS_ORDER) AS SYS_ORD, SH_ORDER
		FROM
			dbo.SystemNetTable a
		...
		GROUP BY SH_SHORT_NAME, TSYS_PROBLEM, CASE SN_GROUP WHEN 'ОВМ' THEN '1/с' ELSE SN_GROUP END, SH_ID, TSYS_GROUP, TSYS_PROBLEM, SH_ORDER
