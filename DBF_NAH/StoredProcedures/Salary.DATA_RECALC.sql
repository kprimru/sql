...
GO
IF OBJECT_ID('[Salary].[DATA_RECALC]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Salary].[DATA_RECALC]  AS SELECT 1')
GO
	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN
				...
				ELSE
				BEGIN
					IF /*@KGS > 70 AND @CL_TERR = 'БГ' AND */@CPS_MIN IS NOT NULL AND @CPS_MIN > @TO_CALC
						SET @TO_CALC = 230
					IF /*@CL_TERR = 'УТ' AND */@CPS_MIN IS NOT NULL AND @CPS_MIN > @TO_CALC
						SET @TO_CALC = 230
				END
			END
		END
		...
		SET @TO_RESULT = ROUND(ROUND(@TO_CALC * @KOB, 0) * IsNUll(@TO_RANGE, 1) * IsNUll(@TO_SERVICE_COEF, 1), 0)
