...
GO
IF OBJECT_ID('[USR].[COMPLECT_INFO_BANK]', 'P ') IS NULL EXEC('CREATE PROCEDURE [USR].[COMPLECT_INFO_BANK]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [USR].[COMPLECT_INFO_BANK]
	@UF_ID	INT
AS
BEGIN
	SET NOCOUNT ON;
		...
		*/
		...
		SELECT DISTINCT
			...
			CONVERT(VARCHAR(20), UIU_DATE, 104) + ' ' +
				...
				[dbo].[WeekDateName](UIU_DATE) + ')' AS UIU_DAY,
...
GO
GRANT EXECUTE ON [USR].[COMPLECT_INFO_BANK] TO rl_tech_info;
GO
