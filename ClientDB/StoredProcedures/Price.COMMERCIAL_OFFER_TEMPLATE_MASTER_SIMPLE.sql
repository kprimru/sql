...
GO
IF OBJECT_ID('[Price].[COMMERCIAL_OFFER_TEMPLATE_MASTER_SIMPLE]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Price].[COMMERCIAL_OFFER_TEMPLATE_MASTER_SIMPLE]  AS SELECT 1')
GO

CREATE OR ALTER PROCEDURE [Price].[COMMERCIAL_OFFER_TEMPLATE_MASTER_SIMPLE]
	@ID	UNIQUEIDENTIFIER
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		IF @MON_CNT IS NULL
			SELECT @MON_CNT = MAX(MON_CNT)
			FROM Price.CommercialOfferDetail
			WHERE ID_OFFER = @ID
