...
GO
IF OBJECT_ID('[Control].[CLIENT_CONTROL_SELECT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Control].[CLIENT_CONTROL_SELECT]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [Control].[CLIENT_CONTROL_SELECT]
	@CLIENT	INT
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		SELECT
			...
			END) AS CAN_REMOVE,
			CASE
				WHEN REMOVE_AUTHOR = 1 AND REMOVE_GROUP = 1 THEN 'АВТОР + ГРУППА'
				WHEN REMOVE_AUTHOR = 0 AND REMOVE_GROUP = 1 THEN 'ГРУППА'
				WHEN REMOVE_AUTHOR = 1 AND REMOVE_GROUP = 0 THEN 'АВТОР'
				ELSE ''
			END AS REMOVE_INFO,
			REMOVE_NOTE
		FROM
