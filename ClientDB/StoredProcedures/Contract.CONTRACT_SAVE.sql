...
BEGIN
	...
	SELECT @NUM_S = CONVERT(NVARCHAR(16), @YEAR) + '-' + PREFIX + ' ' + REPLICATE('0', 4 - LEN(CONVERT(NVARCHAR(16), @NUM))) + CONVERT(NVARCHAR(16), @NUM)
	FROM Contract.Type
	WHERE ID = @TYPE
	...
	BEGIN
		...
		INSERT INTO Contract.Contract(NUM, NUM_S, ID_TYPE, ID_VENDOR, REG_DATE, DATE, ID_YEAR, NOTE, ID_CLIENT, CLIENT, ID_STATUS, LAW, DateFrom, DateTo)
			OUTPUT inserted.ID INTO @TBL
			SELECT @NUM, @NUM_S, @TYPE, @VENDOR, @DATE, @DATE, @ID_YEAR, @NOTE, @ID_CLIENT, @CLIENT, @DEF_STATUS, @LAW, @DATE_S, @DATE_F
