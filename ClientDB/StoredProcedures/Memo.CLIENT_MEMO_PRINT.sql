...
GO
CREATE OR ALTER PROCEDURE [Memo].[CLIENT_MEMO_PRINT]
	@ID	UNIQUEIDENTIFIER
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		SELECT
			...
			c.SHORT AS VENDOR,
			ISNULL('с ' + CONVERT(VARCHAR(20), START, 104), '') + ISNULL(' по ' + CONVERT(VARCHAR(20), FINISH, 104), '') AS DATE,
			Common.MoneyShort(MONTH_PRICE) + ' (в т.ч. НДС)' AS [MONTH],
			Common.MoneyShort(PERIOD_PRICE) + ' (в т.ч. НДС)' AS PERIOD,
			Common.MoneyShort(PERIOD_FULL_PRICE) + ' (в т.ч. НДС)' AS PERIOD_FULL,
			'с ' + CONVERT(VARCHAR(20), PERIOD_START, 104) + ' по ' + CONVERT(VARCHAR(20), PERIOD_END, 104) AS PERIOD_STR,
			e.PayTypeName AS CONTRACT_PAY, g.ContractPayName AS CONTRACT_PAY_NAME,
			FRAMEWORK, DOCUMENTS, CASE LETTER_CANCEL WHEN 1 THEN 'Да' ELSE 'Нет' END AS LETTER_CANCEL, SYSTEMS,
			ISNULL(
				(
