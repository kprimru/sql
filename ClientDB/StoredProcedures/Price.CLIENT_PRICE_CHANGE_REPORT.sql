...
GO
CREATE OR ALTER PROCEDURE [Price].[CLIENT_PRICE_CHANGE_REPORT]
				...
				(
					SELECT
						...
						DSS_REPORT * CASE
							...
							ELSE
								ROUND([dbo].[DistrPrice](op.PRICE, op.[DistrCoef], op.[DistrCoefRound]) * (100 - ISNULL(DF_DISCOUNT, 0)) / 100, 2)
						END AS OLD_PRICE,
						DSS_REPORT * CASE
							WHEN ISNULL(DF_FIXED_PRICE, 0) <> 0 THEN DF_FIXED_PRICE
							ELSE
								ROUND([dbo].[DistrPrice](np.PRICE, np.[DistrCoef], np.[DistrCoefRound]) * (100 - ISNULL(DF_DISCOUNT, 0)) / 100, 2)
						END AS NEW_PRICE
					FROM
						#client
