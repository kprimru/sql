...
GO
CREATE PROCEDURE [USR].[SERVICE_COMMON_GRAPH_NEW]
	@SERVICE	INT,
	@BEGIN		SMALLDATETIME,
	@END		SMALLDATETIME,
...
BEGIN
	...
	DECLARE @WEEK TABLE
		(
			WEEK_ID	INT PRIMARY KEY CLUSTERED,
	...
	DECLARE @usr TABLE
		(
			...
			PRIMARY KEY CLUSTERED (ClientID, ResVersion, ConsExeVersion)
		)
	...
	INSERT INTO @usr(ClientID, ResVersion, ConsExeVersion)
		...
		FROM 
			...
			LEFT OUTER JOIN dbo.RegNodeTable x ON x.SystemName = SystemBaseName AND z.UD_DISTR = x.DistrNumber AND z.UD_COMP = x.CompNumber
		WHERE ISNULL(Service, 0) = 0
	...
	DECLARE @update TABLE
		(
			...
			WD					CHAR(2),
			PRIMARY KEY CLUSTERED (ClientID, UpdateDateTime)
		)	
	...
	DECLARE @usrdata TABLE
		(
			...
			UIU_DOCS			INT,
			InfoBankID			INT,
			PRIMARY KEY CLUSTERED (UD_ID_CLIENT, UI_DISTR, InfoBankId, UI_COMP, ID)		
		)
	...
	DECLARE @compl TABLE
		(
			...
			WeekID		INT,
			Comp		VARCHAR(MAX),
			PRIMARY KEY CLUSTERED (ClientId, WeekId)
		)
	...
	DECLARE @actual TABLE
		(
			...
			WeekID		INT,
			Actual		VARCHAR(800),
			PRIMARY KEY CLUSTERED (ClientID, WeekId, Actual)
		)
						...
						(
							...
							FROM 
								(
									...
									SELECT DISTINCT ID_CLIENT, NULL, DISTR, COMP, 'СвРег', 1000 AS InfoBankOrder, z.SystemOrder
									FROM 
										dbo.ClientDistrView z WITH(NOEXPAND)
										INNER JOIN dbo.SystemBanksView x WITH(NOEXPAND) ON z.SystemID = x.SystemID
									...
									SELECT DISTINCT ID_CLIENT, NULL, DISTR, COMP, 'Рег', 1000 AS InfoBankOrder, z.SystemOrder
									FROM 
										dbo.ClientDistrView z WITH(NOEXPAND)
										INNER JOIN dbo.SystemBanksView x WITH(NOEXPAND) ON z.SystemID = x.SystemID
