...
GO
IF OBJECT_ID('[Din].[DIN_FILES_SELECT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Din].[DIN_FILES_SELECT]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [Din].[DIN_FILES_SELECT]
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		SELECT
			CASE DF_RIC
				WHEN 20 THEN ''
				ELSE 'РИЦ ' + CONVERT(VARCHAR(20), DF_RIC) + '\'
			END + NT_NAME +
			CASE NT_NOTE
				WHEN '' THEN ''
				ELSE ', ' + NT_NOTE
