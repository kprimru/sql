...
GO
IF OBJECT_ID('[Seminar].[WEB_MAIL_PROFILE_SELECT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Seminar].[WEB_MAIL_PROFILE_SELECT]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [Seminar].[WEB_MAIL_PROFILE_SELECT]
		...
		WHERE S.[WEB] = 1
			...
			AND
				(
					...
					OR
					@ID IS NULL
					AND P.[PROFILE_DATE] IS NULL
					AND GetDate() > S.[PROFILE_DATE]
				);
