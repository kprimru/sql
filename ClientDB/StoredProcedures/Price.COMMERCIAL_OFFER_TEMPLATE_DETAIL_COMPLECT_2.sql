...
GO
IF OBJECT_ID('[Price].[COMMERCIAL_OFFER_TEMPLATE_DETAIL_COMPLECT_2]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Price].[COMMERCIAL_OFFER_TEMPLATE_DETAIL_COMPLECT_2]  AS SELECT 1')
GO

CREATE OR ALTER PROCEDURE [Price].[COMMERCIAL_OFFER_TEMPLATE_DETAIL_COMPLECT_2]
	@ID	UNIQUEIDENTIFIER
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		WHERE ID_OFFER = @ID AND Variant = 2
