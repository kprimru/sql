...
GO
IF OBJECT_ID('[Report].[EXPERT_QUESTION_STAT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Report].[EXPERT_QUESTION_STAT]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [Report].[EXPERT_QUESTION_STAT]
			...
			(
				SELECT
					...
					(
						SELECT COUNT(*)
						FROM dbo.ClientDutyQuestion
						WHERE DATE >= DATEADD(WEEK, -1, START) AND DATE < DATEADD(WEEK, -1, FINISH)
					) AS CNT_PREV,
					(
						SELECT COUNT(*)
						FROM
...
END
GO
GRANT EXECUTE ON [Report].[EXPERT_QUESTION_STAT] TO rl_report;
GO
