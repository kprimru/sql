...
GO
IF OBJECT_ID('[Seminar].[WEB_MAIL_INVITE_SELECT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Seminar].[WEB_MAIL_INVITE_SELECT]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [Seminar].[WEB_MAIL_INVITE_SELECT]
...
BEGIN
	...
	BEGIN TRY
		...
		WHERE S.[WEB] = 1
			...
			AND P.[STATUS] = 1
			AND T.[Code] = 'SEMINAR' -- ToDo должно настраиваться
			AND
				(
					P.ID = @ID
					OR
