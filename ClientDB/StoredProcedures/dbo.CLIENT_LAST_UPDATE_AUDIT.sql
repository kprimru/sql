...
GO
IF OBJECT_ID('[dbo].[CLIENT_LAST_UPDATE_AUDIT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[CLIENT_LAST_UPDATE_AUDIT]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [dbo].[CLIENT_LAST_UPDATE_AUDIT]
	@SERVICE	INT,
	@MANAGER	INT,
	@DATE		SMALLDATETIME = NULL,
	@WeekCount  SmallInt = NULL
AS
BEGIN
	SET NOCOUNT ON;
		...
		WHERE EXISTS
			(
				...
				WHERE C.UD_ID = U.UD_ID
					...
					AND UIU_DATE_S BETWEEN DATEADD(WEEK, -@WeekCount, @LAST_DATE) AND @LAST_DATE
			);
