...
GO
IF OBJECT_ID('[Price].[COMMERCIAL_OFFER_TEMPLATE_MASTER_OPTIMIZE]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Price].[COMMERCIAL_OFFER_TEMPLATE_MASTER_OPTIMIZE]  AS SELECT 1')
GO

CREATE OR ALTER PROCEDURE [Price].[COMMERCIAL_OFFER_TEMPLATE_MASTER_OPTIMIZE]
	@ID	UNIQUEIDENTIFIER
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		SELECT @DIR_POS = DIRECTOR_POS, @SURNAME = PER_SURNAME, @NAME = PER_NAME, @PATRON = PER_PATRON, @MON = [dbo].[MonthDateName](DATE)
		...
		SELECT
			...
			@DIR_POS AS DIRECTOR_POS,
			[Surname] = @SURNAME,
			[Name] = @NAME,
			[Patron] = @Patron,
			[Manager] = @MANAGER,
			[DIR_SHORT_ROD] = @DIR_SHORT_ROD,
			...
			@MON AS CL_MONTH, DATEPART(DAY, DATE) AS CL_DAY, DATEPART(YEAR, DATE) AS CL_YEAR,
			(
				SELECT TOP 1 y.NAME
				FROM
					Price.CommercialOfferDetail z
			...
			Common.MoneyFormat((
				SELECT SUM(DELIVERY_PRICE + SUPPORT_PRICE + IsNull(CONNECT_PRICE, 0))
