...
GO
CREATE OR ALTER PROCEDURE [USR].[COMPLECT_DETAIL]
	@UF_ID	INT
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN TRY
		...
		SELECT
			...
			T.UF_USERNAME, T.UF_COMPNAME,
			T.UF_WORK_DIR_PATH + ' (' + dbo.FileSizeToStr(T.UF_WORK_DIR_FREE) + ')' AS UF_WORK_DIR, CASE WHEN T.UF_WORK_DIR_FREE <= 1000 THEN 2 WHEN T.UF_WORK_DIR_FREE > 1000 AND T.UF_WORK_DIR_FREE <= 2000 THEN 1 ELSE 0 END AS UF_WORK_DIR_ERROR,
			T.UF_WORK_DIR_SELECTED,
			T.UF_RECEIVE_DIR_PATH + ' (' + dbo.FileSizeToStr(T.UF_RECEIVE_DIR_FREE) + ')' AS UF_RECEIVE_DIR, CASE WHEN T.UF_RECEIVE_DIR_FREE <= 1000 THEN 2 WHEN T.UF_RECEIVE_DIR_FREE > 1000 AND T.UF_RECEIVE_DIR_FREE <= 2000 THEN 1 ELSE 0 END AS UF_RECEIVE_DIR_ERROR,
			T.UF_RECEIVE_DIR_ANS_EXISTS, T.UF_RECEIVE_DIR_CD_KEY
		FROM
			USR.USRFile F
