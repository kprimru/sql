...
GO
IF OBJECT_ID('[Maintenance].[GLOBAL_SETTING_SAVE]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Maintenance].[GLOBAL_SETTING_SAVE]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [Maintenance].[GLOBAL_SETTING_SAVE]
    ...
    @CONTRACT_OLD                   BIT,
	@REPOSITORY_PATH				NVARCHAR(512),
	@PROTOCOL_PATH					NVARCHAR(512),
	@CONFIG_PATH					NVARCHAR(512),
	@CLIENT_AUTO_CLAIM_TYPES        NVARCHAR(512),
	@CLIENT_DUTY_DATE				NVARCHAR(10)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
		...
		UPDATE Maintenance.GlobalSettings
		SET GS_VALUE = @REPOSITORY_PATH
		WHERE GS_NAME = 'REPOSITORY_PATH'

		IF @@ROWCOUNT = 0
			INSERT INTO Maintenance.GlobalSettings(GS_NAME, GS_VALUE, GS_NOTE)
				SELECT 'REPOSITORY_PATH', @REPOSITORY_PATH, ''

		UPDATE Maintenance.GlobalSettings
		SET GS_VALUE = @PROTOCOL_PATH
		WHERE GS_NAME = 'PROTOCOL_PATH'

		IF @@ROWCOUNT = 0
			INSERT INTO Maintenance.GlobalSettings(GS_NAME, GS_VALUE, GS_NOTE)
				SELECT 'PROTOCOL_PATH', @PROTOCOL_PATH, 'Путь к файлу протокола'

		UPDATE Maintenance.GlobalSettings
		SET GS_VALUE = @CONFIG_PATH
		WHERE GS_NAME = 'CONFIG_PATH'

		IF @@ROWCOUNT = 0
			INSERT INTO Maintenance.GlobalSettings(GS_NAME, GS_VALUE, GS_NOTE)
				SELECT 'CONFIG_PATH', @CONFIG_PATH, 'Путь к конфигу загрузки'

		UPDATE Maintenance.GlobalSettings
		SET GS_VALUE = @CLIENT_AUTO_CLAIM_TYPES
		WHERE GS_NAME = 'CLIENT_AUTO_CLAIM_TYPES'

		IF @@ROWCOUNT = 0
			INSERT INTO Maintenance.GlobalSettings(GS_NAME, GS_VALUE, GS_NOTE)
				SELECT 'CLIENT_AUTO_CLAIM_TYPES', @CLIENT_AUTO_CLAIM_TYPES, ''

		UPDATE Maintenance.GlobalSettings
				SET GS_VALUE = Convert(NVARCHAR(11), @CLIENT_DUTY_DATE, 104)
				WHERE GS_NAME = 'CLIENT_DUTY_DATE'

		IF @@ROWCOUNT = 0
			INSERT INTO Maintenance.GlobalSettings(GS_NAME, GS_VALUE, GS_NOTE)
				SELECT 'CLIENT_DUTY_DATE', Convert(NVARCHAR(11), @CLIENT_DUTY_DATE, 104), ''
