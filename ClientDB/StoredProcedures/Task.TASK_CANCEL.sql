...
GO
IF OBJECT_ID('[Task].[TASK_CANCEL]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Task].[TASK_CANCEL]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [Task].[TASK_CANCEL]
	@ID		UNIQUEIDENTIFIER,
	@NOTE	NVARCHAR(MAX)
AS
BEGIN
	...
	BEGIN TRY
		...
		SET EXEC_DATE = GETDATE(),
			...
			ID_STATUS = (SELECT ID FROM Task.TaskStatus WHERE PSEDO = N'CANCEL')
		WHERE ID = @ID
...
END
GO
GRANT EXECUTE ON [Task].[TASK_CANCEL] TO rl_task_w;
GO
