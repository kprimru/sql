...
GO
IF OBJECT_ID('[Din].[DIN_SEARCH]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Din].[DIN_SEARCH]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [Din].[DIN_SEARCH]
...
BEGIN
	...
	BEGIN TRY
		...
		-----------------------------ДЕЛАЕТ СТАРЫЕ ЗАМЕНЕННЫЕ ДИСТРИБУТИВЫ НЕАКТИВНЫМИ И СНИМАЕТ С НИХ ГАЛОЧКУ
		UPDATE #distr
		SET CHECKED = 0,
			DS_INDEX = 1,
			MASTER_ID = 1
		...
		-------------------------------ВЫБОР ПЕРВОГО ПОПАВШЕГОСЯ АКТИВНОГО КОМПЛЕКТА ПРИ ПЕРЕХОДЕ ИЗ КАРТОЧКИ----------------
