...
GO
IF OBJECT_ID('[Reg].[ONLINE_DISTR_SELECT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Reg].[ONLINE_DISTR_SELECT]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [Reg].[ONLINE_DISTR_SELECT]
	@REGISTER	BIT,
	@FREE		BIT,
	@MAIN		BIT,
	@DISTR		INT,
...
BEGIN
	...
	BEGIN TRY
		...
		WHERE (@NAME IS NULL OR e.Comment LIKE @NAME) AND c.SST_SHORT = 'ДСП'
