...
GO
CREATE OR ALTER PROCEDURE [USR].[PROCESS_FILE]
	@t				NVarChar(Max),
	@md5			VarChar(100),
	@hash			VarChar(100),
	@filename		VarCHar(50),
...
BEGIN
	...
	BEGIN TRY
		...
		(
			...
			WorkDirPath 				VarChar(256),
			WorkDirFree					BigInt,
			WorkDirSelected 			VarChar(256),
			ReceiveDirPath 				VarChar(256),
			ReceiveDirFree				BigInt,
			ReceiveDirAnsExists 		VarChar(256),
			ReceiveDirCdKey		 		VarChar(256),
			ConsTmpDirPath 				VarChar(256),
			ConsTmpDirFree				BigInt
		);
		...
		(
			...
			Rgt, UserName, CompName, WorkDirPath, WorkDirFree, WorkDirSelected,
			ReceiveDirPath, ReceiveDirFree, ReceiveDirAnsExists, ReceiveDirCdKey
		)
		SELECT
			@Client_Id,
			...
			c.value('(consadmin[1]/UserName)[1]',											'VarChar(100)')	AS UserName,
			c.value('(consadmin[1]/CompName)[1]',											'VarChar(100)')	AS CompName,

			c.value('(tech_info[1]/ConsWorkDir/Path)[1]',									'VarChar(256)')	AS ConsWorkDir,
			c.value('(tech_info[1]/ConsWorkDir/FreeSpace)[1]',								'VarChar(20)')	AS ConsWorkFree,
			c.value('(tech_info[1]/ConsWorkDir/SelectedBy)[1]',								'VarChar(256)')	AS ConsWorkSelected,

			c.value('(tech_info[1]/ReceiveDir/Path)[1]',									'VarChar(256)')	AS ReceiveDir,
			c.value('(tech_info[1]/ReceiveDir/FreeSpace)[1]',								'VarChar(20)')	AS ReceiveFree,
			c.value('(tech_info[1]/ReceiveDir/Remaining_ANS_files/@exist)[1]',				'VarChar(20)')	AS ReceiveExistsFree,
			c.value('(tech_info[1]/ReceiveDir/Remaining_ANS_files/@RECEIVE_CD_KEY)[1]',		'VarChar(20)')	AS ReceiveCDKey
		...
		INSERT INTO USR.USRFileTech(
						...
						UF_RGT, UF_USERNAME, UF_COMPNAME,
						UF_WORK_DIR_PATH, UF_WORK_DIR_FREE, UF_WORK_DIR_SELECTED,
						UF_RECEIVE_DIR_PATH, UF_RECEIVE_DIR_FREE, UF_RECEIVE_DIR_ANS_EXISTS, UF_RECEIVE_DIR_CD_KEY
						)
		SELECT
			...
			Rgt,
			UserName, CompName, WorkDirPath, WorkDirFree, WorkDirSelected,
			ReceiveDirPath, ReceiveDirFree, ReceiveDirAnsExists, ReceiveDirCdKey
		FROM @Usr
