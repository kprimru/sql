...
GO
CREATE OR ALTER PROCEDURE [dbo].[CLIENT_DELIVERY_FILTER]
...
BEGIN
	...
	BEGIN TRY
		...
		WHERE (a.ID_DELIVERY = @DELIVERY OR @DELIVERY IS NULL)
			...
			AND (b.ServiceID IN (SELECT ID FROM TableIDFromXML(@SERVICE)) OR @SERVICE IS NULL)
			AND (b.ManagerID IN (SELECT ID FROM TableIDFromXML(@MANAGER)) OR @MANAGER IS NULL)
