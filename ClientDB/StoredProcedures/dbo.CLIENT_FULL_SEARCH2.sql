...
GO
IF OBJECT_ID('[dbo].[CLIENT_FULL_SEARCH2]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[CLIENT_FULL_SEARCH2]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [dbo].[CLIENT_FULL_SEARCH2]
	...
	BEGIN TRY
		...
		BEGIN
			...
			IF @SERVICE IS NOT NULL OR @ISLARGE IS NOT NULL OR @KIND IS NOT NULL OR @STATUS IS NOT NULL OR @SERVICE_TYPE IS NOT NULL OR @TYPE IS NOT NULL OR @ORI = 1 BEGIN
				...
				WHERE (ClientServiceID = @SERVICE OR @SERVICE IS NULL)
					...
					AND (StatusID IN (SELECT Item FROM dbo.GET_TABLE_FROM_LIST(@STATUS, ',')) OR @STATUS IS NULL)
		...
		BEGIN
			...
			IF @STATUS IS NOT NULL OR @SERVICE IS NOT NULL BEGIN
				...
				SELECT ClientID, @FilterType_COMMON
				FROM dbo.ClientTable
				WHERE (StatusID IN (SELECT Item FROM dbo.GET_TABLE_FROM_LIST(@STATUS, ',')) OR @STATUS IS NULL)
