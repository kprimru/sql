...
GO
CREATE OR ALTER PROCEDURE [Report].[DISCOUNT_SA_SK]
			...
			CONVERT(DECIMAL(8, 2),
				CASE
					WHEN (ISNULL(DF_FIXED_PRICE, 0) <> 0) THEN
						CONVERT(DECIMAL(8, 2), ROUND((100 * ([dbo].[DistrPrice](PRICE, COEF, RND) - DF_FIXED_PRICE) / NULLIF([dbo].[DistrPrice](PRICE, COEF, RND), 0)), 2))
					WHEN DF_ID_PRICE = 6 THEN
						CONVERT(DECIMAL(8, 2), ROUND((100 * ([dbo].[DistrPrice](PRICE, COEF, RND) - DEPO_PRICE) / NULLIF([dbo].[DistrPrice](PRICE, COEF, RND), 0)), 2))
