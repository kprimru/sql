...
GO
CREATE OR ALTER PROCEDURE [Report].[DBF_RANGE_COMPARE]
...
BEGIN
    ...
    FROM [DBF].dbo.TOTable AS T
    INNER JOIN @DBFServices AS S ON S.Id = T.TO_ID_COUR
    CROSS APPLY
    (
        SELECT TOP (1)
            SYS_REG_NAME, DIS_NUM, DIS_COMP_NUM
        FROM [DBF].dbo.TODistrTable AS TD
        INNER JOIN [DBF].dbo.DistrView AS D WITH(NOEXPAND) ON D.DIS_ID = TD.TD_ID_DISTR
        LEFT JOIN [DBF].dbo.RegNodeTable AS R ON D.SYS_REG_NAME = R.RN_SYS_NAME AND D.DIS_NUM = R.RN_DISTR_NUM AND D.DIS_COMP_NUM = R.RN_COMP_NUM
