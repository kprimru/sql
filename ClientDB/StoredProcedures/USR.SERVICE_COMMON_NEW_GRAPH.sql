...
GO
IF OBJECT_ID('[USR].[SERVICE_COMMON_NEW_GRAPH]', 'P ') IS NULL EXEC('CREATE PROCEDURE [USR].[SERVICE_COMMON_NEW_GRAPH]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [USR].[SERVICE_COMMON_NEW_GRAPH]
	@SERVICE	INT,
	@BEGIN		SMALLDATETIME,
	@END		SMALLDATETIME,
...
BEGIN
	...
	BEGIN TRY
		...
		-- тут несколько костыльно поределяем к какому комплекту принадлежит пополнение. Вполне может быть коллизия, если у клиента
