...
GO
CREATE OR ALTER PROCEDURE [dbo].[SEMINAR_REPORT]
		...
		FROM
			(
				SELECT
					...
					[dbo].[MonthDateName](SEM_BEGIN) + ' ' + CONVERT(VARCHAR(20), DATEPART(YEAR, SEM_BEGIN)) SEM_BEGIN_STR,
					[dbo].[MonthDateName](SEM_END) + ' ' + CONVERT(VARCHAR(20), DATEPART(YEAR, SEM_END)) AS SEM_END_STR
				FROM
					(
						SELECT
