...
GO
IF OBJECT_ID('[dbo].[ACT_CALC_SELECT]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[ACT_CALC_SELECT]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [dbo].[ACT_CALC_SELECT]
	@SERVICE	INT,
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			...
			CASE
				WHEN BILL IS NULL THEN 'НЕТ СЧЕТА (' + CONVERT(VARCHAR(20), MON_DATE, 104) + ')'
				WHEN BILL = INCOME THEN ''
				ELSE 'Не оплачено ' + CONVERT(NVARCHAR(64), ROUND(100 * (BILL - ISNULL(INCOME, 0)) / BILL, 2)) + '% суммы'
			END AS PAY_COMMENT,
