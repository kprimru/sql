...
GO
IF OBJECT_ID('[dbo].[STT_TOTAL_REPORT_SUBHOST]', 'P ') IS NULL EXEC('CREATE PROCEDURE [dbo].[STT_TOTAL_REPORT_SUBHOST]  AS SELECT 1')
GO
CREATE OR ALTER PROCEDURE [dbo].[STT_TOTAL_REPORT_SUBHOST]
	@BEGIN		SMALLDATETIME,
	@END		SMALLDATETIME,
	@SERVICE	INT = NULL,
	@DETAIL		BIT = 0
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO #cl(ClientID, Service, Manager, STT_COUNT, STT_CHECK, Subhost)
		SELECT
			c.ClientID,
			ISNULL(
...
END
GO
GRANT EXECUTE ON [dbo].[STT_TOTAL_REPORT_SUBHOST] TO rl_stt_report;
GO
