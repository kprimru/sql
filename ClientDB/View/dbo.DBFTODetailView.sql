...
AS
	...
	FROM
		...
		LEFT JOIN [DBF].[dbo.TOAddressTable] ON TA_ID_TO = TO_ID
		LEFT JOIN [DBF].[dbo.StreetTable] a ON ST_ID = TA_ID_STREET
		LEFT JOIN [DBF].[dbo.CityTable] b ON CT_ID = ST_ID_CITY
		LEFT JOIN [DBF].[dbo.ActivityTable] ON AC_ID = CL_ID_ACTIVITY
		CROSS APPLY
			(
