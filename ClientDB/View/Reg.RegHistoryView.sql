...
CREATE NONCLUSTERED INDEX [IX_Reg.RegHistoryView(ID_DISTR,DATE)+(COMMENT,COMPLECT,DS_NAME,DS_REG,FIRST_REG,ID,NT_SHORT,REG_DATE,SST_SHORT,SUBHOST,SystemSho] ON [Reg].[RegHistoryView] ([ID_DISTR] ASC, [DATE] ASC) INCLUDE ([COMMENT], [COMPLECT], [DS_NAME], [DS_REG], [FIRST_REG], [ID], [NT_SHORT], [REG_DATE], [SST_SHORT], [SUBHOST], [SystemShortName], [TRAN_COUNT], [TRAN_LEFT]);
GO
