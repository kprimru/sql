...
GO
CREATE OR ALTER VIEW [dbo].[ClientView]
WITH SCHEMABINDING
AS
	SELECT
...
CREATE NONCLUSTERED INDEX [IX_dbo.ClientView(ManagerID,ServiceStatusID)+(ClientFullName,ClientID,ManagerName,ServiceID,ServiceName,ServiceStatusName,Servic] ON [dbo].[ClientView] ([ManagerID] ASC, [ServiceStatusID] ASC) INCLUDE ([ClientFullName], [ClientID], [ManagerName], [ServiceID], [ServiceName], [ServiceStatusName], [ServiceTypeID]);
...
CREATE NONCLUSTERED INDEX [IX_dbo.ClientView(ServiceID,ServiceStatusID)+(ClientFullName,ClientID,ManagerID,ManagerName,ServiceName,ServiceTypeID,ServiceSta] ON [dbo].[ClientView] ([ServiceID] ASC, [ServiceStatusID] ASC) INCLUDE ([ClientFullName], [ClientID], [ManagerID], [ManagerName], [ServiceName], [ServiceTypeID], [ServiceStatusName]);
CREATE NONCLUSTERED INDEX [IX_dbo.ClientView(ServiceLogin)+(ClientID)] ON [dbo].[ClientView] ([ServiceLogin] ASC) INCLUDE ([ClientID]);
CREATE NONCLUSTERED INDEX [IX_dbo.ClientView(ServiceStatusID,ServiceID,ManagerID)+(ClientFullName,ClientID,ManagerName,ServiceName,ServiceStatusName,Servic] ON [dbo].[ClientView] ([ServiceStatusID] ASC, [ServiceID] ASC, [ManagerID] ASC) INCLUDE ([ClientFullName], [ClientID], [ManagerName], [ServiceName], [ServiceStatusName], [ServiceTypeID]);
