...
GO
CREATE OR ALTER VIEW [dbo].[ClientDistrView]
WITH SCHEMABINDING
AS
	SELECT
		a.ID, a.ID_CLIENT,
...
CREATE NONCLUSTERED INDEX [IX_dbo.ClientDistrView(ID_CLIENT,DS_REG,DistrTypeBaseCheck,SystemBaseCheck)+(COMP,DISTR,DistrStr,DistrTypeID,DistrTypeName,DS_ID] ON [dbo].[ClientDistrView] ([ID_CLIENT] ASC, [DS_REG] ASC, [DistrTypeBaseCheck] ASC, [SystemBaseCheck] ASC) INCLUDE ([COMP], [DISTR], [DistrStr], [DistrTypeID], [DistrTypeName], [DS_ID], [DS_INDEX], [HostID], [SystemBaseName], [SystemID], [SystemOrder], [SystemTypeID], [SystemTypeName], [DS_NAME], [SystemReg], [SystemShortName], [ID]);
GO
