...
GO
IF OBJECT_ID('[Memo].[MemoPriceSplit]', 'TF') IS NULL EXEC('CREATE FUNCTION [Memo].[MemoPriceSplit] () RETURNS @output TABLE(Id Int) AS BEGIN RETURN END')
GO
CREATE OR ALTER FUNCTION [Memo].[MemoPriceSplit]
(
	@LIST		NVARCHAR(MAX),
...
BEGIN
			...
			CASE
				...
				ELSE CONVERT(MONEY,
						...
						FLOOR(
							...
							SUM(CNT * TOTAL_PRICE) OVER(PARTITION BY 1) / @TotalRate / CNT/*,
	...
	INSERT INTO @TBL(SystemID, DistrTypeID, CNT, PRICE, TAX_PRICE, TOTAL_PRICE)
		...
		FROM
			(
				SELECT SystemID, DistrTypeID, CNT, CONVERT(MONEY, ROUND(PRICE_NDS / CNT / @TotalRate, 2)) AS PRICE, PRICE_NDS
				FROM @SPLIT
			) AS o_O
