...
BEGIN
	...
	SELECT
		...
		Common.MoneyShort(MONTH_PRICE) + ' (в т.ч. НДС 18%)' AS [MONTH],
		Common.MoneyShort(PERIOD_PRICE) + ' (в т.ч. НДС 18%)' AS PERIOD,
		Common.MoneyShort(PERIOD_FULL_PRICE) + ' (в т.ч. НДС 18%)' AS PERIOD_FULL,
		'с ' + CONVERT(VARCHAR(20), PERIOD_START, 104) + ' по ' + CONVERT(VARCHAR(20), PERIOD_FINISH, 104) AS PERIOD_STR,
