...
GO
IF OBJECT_ID('[Client].[COMPANY_PROCESS_PHONE_RETURN]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Client].[COMPANY_PROCESS_PHONE_RETURN]  AS SELECT 1')
GO
ALTER PROCEDURE [Client].[COMPANY_PROCESS_PHONE_RETURN]
	...
	BEGIN TRY
		...
		INSERT INTO Client.CompanyProcessJournal(ID_COMPANY, DATE, TYPE, ID_AVAILABILITY, ID_CHARACTER, ID_PERSONAL, MESSAGE)
			...
			FROM
				...
				INNER JOIN Client.CompanyProcessPhoneView c WITH(NOEXPAND) ON a.ID = c.ID
...
END
GO
GRANT EXECUTE ON [Client].[COMPANY_PROCESS_PHONE_RETURN] TO rl_company_process_return_phone;
GO
