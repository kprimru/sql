...
GO
IF OBJECT_ID('[Price].[COMMERCIAL_OFFER_TEMPLATE_MASTER_FLASH]', 'P ') IS NULL EXEC('CREATE PROCEDURE [Price].[COMMERCIAL_OFFER_TEMPLATE_MASTER_FLASH]  AS SELECT 1')
GO
ALTER PROCEDURE [Price].[COMMERCIAL_OFFER_TEMPLATE_MASTER_FLASH]
	@ID	UNIQUEIDENTIFIER
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT @DIR_POS = DIRECTOR_POS, @SURNAME = PER_SURNAME, @NAME = PER_NAME, @PATRON = PER_PATRON, @MON = [Common].[MonthDateName](DATE)
	...
	SELECT
		...
		Common.MoneyFormat((
			SELECT SUM(DELIVERY_ORIGIN + SUPPORT_FURTHER)
			FROM Price.CommercialOfferDetail z
			WHERE z.ID_OFFER = a.ID
		)) AS TOTAL_ORIGIN,
		...
		(
			SELECT TOP 1 y.NAME
			FROM
				Price.CommercialOfferOther z
				INNER JOIN Price.OtherService y ON z.ID_SERVICE = y.ID
			WHERE z.ID_OFFER = a.ID
			ORDER BY z.ID
		) AS FLASH_NAME,
		Common.MoneyFormat((
			SELECT TOP 1 z.PRICE
			FROM
				Price.CommercialOfferOther z
			WHERE z.ID_OFFER = a.ID
			ORDER BY z.ID
